...
private:
  temporary_buffer(const temporary_buffer&); // 复制构造函数，私有外部无法调用，相当于禁用复制操作
  void operator=(const temporary_buffer&) = delete; // 禁用等号赋值
};
...
temporary_buffer(ForwardIterator first, ForwardIterator last)
{
  try
  {
    len = mystl::distance(first, last);//获取一块区间的长度
    allocate_buffer(); //分配内存，将this->len更新为实际的值
    if (len > 0)
    {
      initialize_buffer(*first, std::is_trivially_default_constructible<T>()); // 判断是否是平凡默认可构造类型 全部使用*first初始化？
    }
  }
  catch (...)
  {
