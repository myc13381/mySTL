...
{
  return comp(lhs, rhs) ? rhs : lhs; //如果lhs.val<rhs.val,comp函数返回true，因此max函数返回rhs
}
// 所以这里函数对象约定的返回值是表达式{第一个参数小于第二个参数}的结果？
...
// 将两个迭代器所指对象对调
/*****************************************************************************************/
// 这里的迭代器为什么是两种类型？-> 不同类型迭代器指向的内容也许是同种类型
...
{
  for (auto n = last - first; n > 0; --n, ++first, ++result) // for (; first != last; ++first, ++result, mystd::ramdom_access_iterator_tag)
  {
    *result = *first;
  }
  return result;
...
{
  return unchecked_copy_cat(first, last, result, iterator_category(first)); // 通过占位参数来进行对应重载版本的调用
}
                   ...
                   mystl::input_iterator_tag)
{
  for (; first != last; ++first, ++result)
  {
    *result = mystl::move(*first); // 配合移动构造函数使用
  }
  return result;
}
