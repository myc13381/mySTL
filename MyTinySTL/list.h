...
// 这个头文件包含了一个模板类 list
// list : 双向链表
// 由于尾结点的存在，此链表本质是一个双向循环链表，但是由于我们并不考虑尾结点的存在，因此此链表逻辑上不成环

// notes:
//
// 异常保证：
// mystl::list<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证(也就是抛出自定义的异常信息)：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
struct list_iterator : public mystl::iterator<mystl::bidirectional_iterator_tag, T>
{
  ...
  pointer   operator->() const { return &(operator*()); } // 先取值，再取地址，所以是返回value的指针
...
private:
  base_ptr  node_;  // 指向末尾节点 尾结点
  size_type size_;  // 大小

public:
  // 构造、复制、移动、析构函数
  list() 
  { fill_init(0, value_type()); } // 内部使用0个元素初始化链表，所以链表为空
  ...
  // 这里记录一下使用copy_init和assign函数对链表初始化的区别。
  // copy_init是完全重新开辟一个新链表，完全从新开始申请内存创建节点
  // assign函数而是尽量利用已有的节点，如果不足则补充节点，如果多余则删除节点
...
public:
  ...
  void     emplace_front(Args&& ...args)
  {
    ...
    link_nodes_at_front(link_node->as_base(), link_node->as_base()); // 插入一个节点的典型操作
    ++size_;
  }
  ...
  void remove_if(UnaryPredicate pred); // 参数是个仿函数，该函数传入的参数是 value_type
...
{
  MYSTL_DEBUG(this != &x); // 不允许自己和自己拼接
  if (!x.empty())
  {
    THROW_LENGTH_ERROR_IF(size_ > max_size() - x.size_, "list<T>'s size too big");

    auto f = x.node_->next; // first
    auto l = x.node_->prev; // last

    x.unlink_nodes(f, l); // 与原来的链表断开连接
    link_nodes(pos.node_, f, l); // 重新连接
...
// 将 it 所指的节点接合于 pos 之前,it是指向链表x中某一节点的迭代器
template <class T>
...
// 移除 list 中满足 pred 为 true 重复元素-->这里是取出连续的重复元素？如果不是连续重复，则不会去除？
template <class T>
template <class BinaryPredicate>
...
void list<T>::merge(list& x, Compare comp)
{
  if (this != &x)
  {
    ...
    while (f1 != l1 && f2 != l2)
    {
      if (comp(*f2, *f1)) // *f2作为comp函数的第一个参数
      {
        // 使 comp 为 true 的一段区间
        auto next = f2;
        ++next;
...
void list<T>::reverse()
{
  ...
  {
    mystl::swap(i.node_->prev, i.node_->next); // 交换两个节点
    i.node_ = i.node_->prev;
  }
  // 反转尾结点，尾结点是前面的结束标志，需要最后处理
...
{
  node_ptr p = node_allocator::allocate(1); // 首先申请内存
  try
  {
    data_allocator::construct(mystl::address_of(p->value), mystl::forward<Args>(args)...); // 在指定内存位置构建对象
    p->prev = nullptr;
    p->next = nullptr;
  }
  catch (...)
...
{
  data_allocator::destroy(mystl::address_of(p->value)); // destroy负责调用析构函数
  node_allocator::deallocate(p);                        // deallocate负责释放内存
}
...
{
  if (pos == node_->next) // 等号左边是const_iterator类型，右边是base_ptr这样比较没有问题吗？
  {
...
void list<T>::fill_assign(size_type n, const value_type& value)
{
  ...
  for (; n > 0 && i != e; --n, ++i)
  {
    *i = value; // 由于重载了运算符，*i返回的是节点内部的值
  }
  // 判断是否存在多余元素或者元素不足保证容器存在n个元素
  if (n > 0)
  {
    insert(e, n, value);
  }
...
list<T>::list_sort(iterator f1, iterator l2, size_type n, Compared comp)
{
  ...
  if (n == 2)
  {
    ...
    {
      auto ln = l2.node_;
      // 通过调用unlink_nodes和link_nodes可以保证链表操作逻辑的正确性无需考虑尾结点node_的影响
      unlink_nodes(ln, ln);
      link_nodes(f1.node_, ln, ln); // 在原来f1.node_的位置插入ln
      return l2;
    }
    else return f1;
  }
  ...
  auto n2 = n / 2;
  auto l1 = f1;
  mystl::advance(l1, n2); // 让迭代器l1前进n2的距离
  // 进行归并排序 --> 似乎理解了迭代器操作一般都设置为左闭右开的原因
  // 尾结点的存在并不影响到最后的排序，因为它根本不会参与到排序之中
  auto result = f1 = list_sort(f1, l1, n2, comp);  // 前半段的最小位置
  auto f2 = l1 = list_sort(l1, l2, n - n2, comp);  // 后半段的最小位置

  // 把较小的一段区间移到前面？ 这么做应该是为了让node_->next指向正确的值而产生一个正确的头部，方便下面合并
  if (comp(*f2, *f1))
  {
    auto m = f2;
    ++m;
    ...
    auto f = f2.node_;
    auto l = m.node_->prev;
    result = f2; // 对result重新赋值，这个值就是我们的返回值
    l1 = f2 = m;
    unlink_nodes(f, l);
    m = f1;
    ++m;
  ...
  // 合并两段有序区间，类似于merge，但是merge是两个链表层面上的合并，这里为了减小开销而单独实现了一遍
  while (f1 != l1 && f2 != l2)
  {
    if (comp(*f2, *f1))
    {
