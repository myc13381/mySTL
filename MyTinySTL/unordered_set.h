...
private:
  // 使用 hashtable 作为底层机制
  typedef hashtable<Key, Hash, KeyEqual> base_type; // hashtable作为基本类型
  base_type ht_;
...
public:
  ...
  unordered_set()
    :ht_(100, Hash(), KeyEqual()) // 默认构造时，hashtable的vector bucket初始长度为101
  {
  }
 ...
 // 使用两个迭代器构造一个unordered_set
    ...
    : ht_(mystl::max(bucket_count, static_cast<size_type>(mystl::distance(first, last))), hash, equal)
  {
    for (; first != last; ++first)
      ht_.insert_unique_noresize(*first); // 由于构造函数已经初始化了空间，所以直接使用_noresize版本的插入函数
  }
  ...
  ~unordered_set() = default; // 默认调用hashtable的析构函数
...
public:
  // 使用友元可以让此函数访问lhs/rhs.ht_成员变量
  // 有一说一没看懂这里比较相等的操作是什么意思
