...
// char_traits 兼容多种字符类型

// 包含字符串的基本操作，如比较，复制，移动，填充
...
// Partialized. char_traits<char> char类型的特化，相关函数直接使用标准库的实现
template <> 
...
// Partialized. char_traits<wchar_t> wchar_t的特化,同样使用标准库提供的方法
template <>
...
// Partialized. char_traits<char16_t> // 和原生模板类实现一样？
template <>
...
// Partialized. char_traits<char32_t> char32_t的特化版本，和模板原生版本也是一样的
template <>
...
public:
  ...
  allocator_type get_allocator() { return allocator_type(); } // allocator直接默认构造
...
public:
  ...
  static constexpr size_type npos = static_cast<size_type>(-1); // 使用-1作为不存在的位置，constexpr表示常量表达式
...
public:
  ...
  // try_init函数会忽略异常(申请内存时产生的)，因此该构造函数不会抛出异常
  ...
  // 使用input_iterator迭代器构造
  template <class Iter, typename std::enable_if<
    mystl::is_input_iterator<Iter>::value, int>::type = 0>
  basic_string(Iter first, Iter last)
  { copy_init(first, last, iterator_category(first)); }

  // 对于这些没有额外template <typename T>的复制构造函数，其参数类型应该与需要构造的类型保持一致，否则编译时期就会报错
  // 用另一个同类型对象的左值来复制构造，复制全部
...
public:
  ...
  { return static_cast<size_type>(-1); } // 最大值 对于无符号整型数类型如size_t来说，-1是非常大的数
  ...
  // 访问元素相关操作 返回值设有引用和常量引用版本 需要注意的是，当使用引用作为返回类型时，返回的对象必须具有有效的生命周期，否则会导致未定义的行为
  // []运算符 at函数，两者的主要区别在于对访问string[size]的处理，而at函数又使用了[]运算符
  reference       operator[](size_type n) 
  {
    MYSTL_DEBUG(n <= size_);
    if (n == size_)
      *(buffer_ + n) = value_type(); // 创建一个基本字符类型的默认对象？0？
    return *(buffer_ + n); 
  }
  ...
  { append(1, ch); } // 在buffer_[size]处添加值
  void     pop_back()
  {
    MYSTL_DEBUG(!empty());
    --size_;
  ...
  // append
  basic_string& append(size_type count, value_type ch); // 最基本的append，在尾部添加一个值，接下来逐渐对append进行封装
  ...
  // erase /clear  内部调用的是move函数
  iterator erase(const_iterator pos);
  iterator erase(const_iterator first, const_iterator last);

  // resize 如果需要变长，调用append;如果是变短，则调用erase(start,end)
  ...
  // compare 内部调用compare_str,并丰富了比较对象的选择
  ...
  // replace 底层是调用的自己实现的replace_cstr,本质是指针与move操作，这里提供了多种方式的replace
  ...
  // 这里全部传入迭代器所以底层调用的是自己实现的replace_copy函数
  template <class Iter, typename std::enable_if<
    mystl::is_input_iterator<Iter>::value, int>::type = 0>
...
public:
  ...
  { return append(1, ch); }
  basic_string& operator+=(const_pointer str)
  { return append(str, str + char_traits::length(str)); } // 参数为两个迭代器
...
operator=(value_type ch)
{
  if (cap_ < 1)
  {
    auto new_buffer = data_allocator::allocate(2); // 第二个位置存放0,代表C语言的字符串
    data_allocator::deallocate(buffer_);
    buffer_ = new_buffer;
    cap_ = 2;
  }
...
// 插入元素操作，主要是条件判断加上fill/move/reallocate_and_fill之类操作
// 在 pos 处插入一个元素
template <class CharType, class CharTraits>
...
insert(const_iterator pos, size_type count, value_type ch)
{
  ...
  char_traits::move(r + count, r, count); //这里有问题 应该是char_traits::move(r + count, r, static_cast<size_t>(end()-pos));
...
insert(const_iterator pos, Iter first, Iter last)
{
  ...
  char_traits::move(r + len, r, len);  // char_traits::move(r + len, r, static_cast<size_t>(end()-pos));
...
reverse() noexcept
{
  ...
  {
    mystl::iter_swap(i++, --j); // 看起来还挺对称的
  }
}

// 交换两个 basic_string,底层是交换内部封装的属性
template <class CharType, class CharTraits>
void basic_string<CharType, CharTraits>::
...
{
  *(buffer_ + size_) = value_type(); // 按照C语言的约定，字符串尾部需要是0,这里直接进行赋值操作，如果cap_==size_会导致数据丢失
  return buffer_;
}
...
reinsert(size_type size)
{
  ...
  buffer_ = new_buffer; // 这里写的是不是有问题？捕获异常之后再执行这些操作是否会导致访问错误的内存
  size_ = size;
  cap_ = size;
}

// append_range，末尾追加一段 [first, last) 内的字符,注意是在尾部添加
template <class CharType, class CharTraits>
template <class Iter>
basic_string<CharType, CharTraits>&
basic_string<CharType, CharTraits>::
...
// 字符串比较，对char_traites::compare函数的进一步封装
template <class CharType, class CharTraits>
int basic_string<CharType, CharTraits>::
...
{
  // 如果count1不合理，则缩小至适当的值
  if (static_cast<size_type>(cend() - first) < count1)
  {
    count1 = cend() - first;
  }
  // 通过比较count1和count2大小来决定是否需要进一步开辟空间
  if (count1 < count2)
  {
    ...
    char_traits::move(r + count2, first + count1, end() - (first + count1)); // 为中间的插入留出足够的空间
    char_traits::copy(r, str, count2); // 替换操作
    size_ += add; // 更新size_
  }
  else
  {
...
// 把 [first, last) 的字符替换成 [first2, last2) 这里传入的参数是迭代器
template <class CharType, class CharTraits>
template <class Iter>
basic_string<CharType, CharTraits>&
basic_string<CharType, CharTraits>::
...
// reallocate 函数 按照需要增加缓冲空间
template <class CharType, class CharTraits>
void basic_string<CharType, CharTraits>::
reallocate(size_type need)
{
  const auto new_cap = mystl::max(cap_ + need, cap_ + (cap_ >> 1)); // 计算新的容量
...
reallocate_and_fill(iterator pos, size_type n, value_type ch)
{
  ...
  const auto new_cap = mystl::max(old_cap + n, old_cap + (old_cap >> 1)); // 需要大小或者1.5倍
  auto new_buffer = data_allocator::allocate(new_cap);
  auto e1 = char_traits::move(new_buffer, buffer_, r) + r; // 复制原来的前面部分
  auto e2 = char_traits::fill(e1, ch, n) + n; // 填充新的值
  char_traits::move(e2, buffer_ + r, size_ - r); // 复制原来的后面部分
...
// 两个bastic_string类型变量拼接，需要考虑其中一个为const属性的情况
template <class CharType, class CharTraits>
basic_string<CharType, CharTraits>
