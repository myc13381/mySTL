...
void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value)
{
  ...
  while (rchild < len)
  {
    ...
    *(first + holeIndex) = *(first + rchild);
    holeIndex = rchild;
    rchild = 2 * (rchild + 1); // rchild=2*rchild+2 找到下一个右孩子，进入循环
  }
  if (rchild == len)
  {  // 如果没有右子节点
  ...
  // 将value值赋值到holeIndex的位置，如果有需要，会上溯一次
