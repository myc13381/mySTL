...
search_n(ForwardIter first, ForwardIter last, Size n, const T& value)
{
  ...
  {
    first = mystl::find(first, last, value); // 找到第一个
    while (first != last)
    {
      auto m = n - 1;
      auto i = first;
                  ...
                  forward_iterator_tag, forward_iterator_tag)
{
  ...
  else
  {
    ...
    while (true)
    {
      // 利用 search 查找某个子序列的首次出现点，找不到则返回 last1，因为是forward迭代器，只能这样做
...
find_end(ForwardIter1 first1, ForwardIter1 last1,
         ForwardIter2 first2, ForwardIter2 last2)
{
  typedef typename iterator_traits<ForwardIter1>::iterator_category Category1; // 获取类型，下同
                ...
                const T& value, forward_iterator_tag)
{
  ...
  auto half = len;
  ForwardIter middle;
  // 折半查找，每次将[fisrt,last)划分为[first,first+half]和[first+half+1,last)两个部分，并比较first+half处的值
  while (len > 0)
  {
    half = len >> 1;
    middle = first;
    mystl::advance(middle, half); // 从第一个元素走half步，走到第half+1个元素处
    if (*middle < value)
    {
      // 因为是forward迭代器，所以下面写成两句而不是first=middle
      first = middle;
      ++first;
                ...
                const T& value, random_access_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    if (*middle < value)
    {
      first = middle + 1; // randomIterator可以随机赋值
      len = len - half - 1;
    }
    else
    {
...
/*****************************************************************************************/
// includes
// 判断序列一S1 是否包含序列二S2 序列需要已经保持升序状态
/*****************************************************************************************/
...
bool is_heap(RandomIter first, RandomIter last)
{
  ...
  for (auto child = 1; child < n; ++child)
  {
    if (first[parent] < first[child])
      return false;
    if ((child & 1) == 0) // 当child是2的倍数时++parent,因为堆是二叉树
      ++parent;
  }
  return true;
}
                ...
                ForwardIter last, forward_iterator_tag)
{
  ...
  auto new_middle = first;   // 迭代器返回的位置,此时的first其实就是新的中间位置
  first2 = middle;
  while (first2 != last)
  {   // 调整剩余元素
    mystl::swap(*first++, *first2++);
...
// rotate_dispatch 的 bidirectional_iterator_tag 版本  通过反转实现顺序的调换
template <class BidirectionalIter>
BidirectionalIter
