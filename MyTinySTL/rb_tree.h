...
NodePtr rb_tree_next(NodePtr node) noexcept
{
  ...
  while (!rb_tree_is_lchild(node)) // 如果自己是右孩子，则一直向上寻找
    node = node->parent;
  return node->parent;
}
...
//         让父节点成为当前节点，再以当前节点为支点左（右）旋 LR/RL--> LL/RR
...
void rb_tree_insert_rebalance(NodePtr x, NodePtr& root) noexcept
{
  ...
  while (x != root && rb_tree_is_red(x->parent))
  {
    ...
    { // 如果父节点是左子节点 LX
      ...
      { // case 3: 父节点和叔叔节点都为红-->祖父设为红色，继续平衡
      ...
      { // 无叔叔节点或叔叔节点为黑可以通过旋转进行平衡 LL直接处理，LR转化为LL然后同样处理
        if (!rb_tree_is_lchild(x))
        { // case 4: 当前节点 x 为右子节点 LR --> LL
          x = x->parent;
          rb_tree_rotate_left(x, root);
        }
        // 都转换成 case 5： 当前节点为左子节点 LL
        rb_tree_set_black(x->parent);
    ...
    { 
      ...
      { // 无叔叔节点或叔叔节点为黑
        if (rb_tree_is_lchild(x))
        { // case 4: 当前节点 x 为左子节点 RL
          x = x->parent;
          rb_tree_rotate_right(x, root);
        }
        // 都转换成 case 5： 当前节点为右子节点 RR
        rb_tree_set_black(x->parent);
        rb_tree_set_red(x->parent->parent);
...
NodePtr rb_tree_erase_rebalance(NodePtr z, NodePtr& root, NodePtr& leftmost, NodePtr& rightmost)
{
  ...
  if (y != z) // z 有两个非空子节点
  {
    ...
    else // y 是 z 的右节点，x节点不需要重新连接
    {
      xp = y;
    }

    // 连接 y 与 z 的父节点 
    if (root == z) // 如果z是头结点，那么替代的y节点也不需要额外处理了
      root = y;
      ...
      z->parent->right = y;
    y->parent = z->parent;
    mystl::swap(y->color, z->color); // 交换颜色
    y = z; // y修改为要删去的节点
  }
  // y == z 说明 z 至多只有一个孩子
  else
  ...
  // --> 如果是z有两个非空子节点，原来的y替换了原来的z，所以真正被删除的是y(因为他被替换上去了并且已经做好了颜色的转化)，这里使用了x替换了y
  // --> 如果z最多只有一个非空节点，则是x替换了z，两种情况均是x替换
  // 此时，y 指向要删除的节点，x 为替代节点，从 x 节点开始调整。
  // 如果删除的节点为红色，树的性质没有被破坏，否则按照以下情况调整（x 为左子节点为例）：
  // case 1: 兄弟节点为红色，令父节点为红，兄弟节点为黑，进行左（右）旋，继续处理 --> 转为2,3,4三种情况之一
  ...
  { // x 为黑色时，调整，否则直接将 x 变为黑色即可
    while (x != root && (x == nullptr || !rb_tree_is_red(x)))
    {
      ...
      { // 如果 x 为左子节点
        ...
        if (rb_tree_is_red(brother)) // 兄弟节点是红色说明父节点必然是黑色
        { // case 1
        ...
        { // case 2  --> brother没有红色子节点
          rb_tree_set_red(brother);
          // rb_tree_set_black(xp) 的设置在最下面
          x = xp;
          xp = xp->parent;
        }
        else 
        { // 此时，brother为黑色，并且brother至少有一个红色子节点
          if (brother->right == nullptr || !rb_tree_is_red(brother->right)) 
          { // case 3  经过上面一系列约束，此时只要brother的右节点不为红色，就可以进入case 3
          ...
          // case 3 的操作是为了转为 case 4
          // 剩下的全是case 4
          // 此时，当前结点颜色是黑-黑色，它的兄弟节点是黑色，但是兄弟节点的右子是红色，兄弟节点左子的颜色任意
          brother->color = xp->color;
          rb_tree_set_black(xp);
          if (brother->right != nullptr)  // brother->right存在才能操作
      ...
      { 
        ...
        { // case 1 兄弟节点为红色，将他与父节点颜色改变，然后选择进入case 1/2/3
          // 进入这种情况，兄弟节点必然有两个黑色子节点，否则兄弟分支无法平衡
          rb_tree_set_black(brother);
          rb_tree_set_red(xp);
        ...
        if ((brother->left == nullptr || !rb_tree_is_red(brother->left)) &&
            (brother->right == nullptr || !rb_tree_is_red(brother->right)))
        { // case 2 如果兄弟节点不存在红色子节点，则将兄弟变为红色，继续调整父节点
          rb_tree_set_red(brother);
          x = xp;
          xp = xp->parent;
        }
    ...
    if (x != nullptr)
      rb_tree_set_black(x); // 将x设置为黑色，因为在这个if作用域内说明删去的y节点是黑色，必然要补充一个黑色节点
  }
  return y;
}

// 模板类 rb_tree
// 参数一代表数据类型，参数二代表键值比较类型(函数对象)
...
public:
  ...
  void      insert_multi(InputIterator first, InputIterator last)
  {
    ...
    for (; n > 0; --n, ++first)
      insert_multi(end(), *first); // 以end()作为hint来进行优化
  }
  ...
  {
    return mystl::pair<iterator, iterator>(lower_bound(key), upper_bound(key)); // ??? upper_bound实现的是否有问题
  }
...
rb_tree(const rb_tree& rhs)
{
  ...
  {
    // 注意这里表达式左边是函数返回值，这里是引用，相当于直接修改了header_节点
...
rb_tree<T, Compare>::get_insert_multi_pos(const key_type& key)
{
  ...
  {
    y = x;
    add_to_left = key_comp_(key, value_traits::get_key(x->get_node_ptr()->value)); // key < x 时返回true，所以当key == x 时进入右子树
...
insert_node_at(base_ptr x, node_ptr node, bool add_to_left)
{
  ...
  rb_tree_insert_rebalance(base_node, root()); // 插入节点后开始调整红黑树
  ++node_count_;
  return iterator(node);
}

// 插入元素，键值允许重复，使用 hint 来尝试减少时间复杂度
template <class T, class Compare>
typename rb_tree<T, Compare>::iterator 
rb_tree<T, Compare>::
...
// 插入元素，键值不允许重复，使用 hint 来尝试减少时间复杂度
template <class T, class Compare>
typename rb_tree<T, Compare>::iterator 
rb_tree<T, Compare>::
...
{
  return lhs.size() == rhs.size() && mystl::equal(lhs.begin(), lhs.end(), rhs.begin()); // 每一个元素单独拿出来比较
}
